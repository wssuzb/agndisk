...
function thin_disk(band::Vector{String}, in_time::Vector{Float64}, in_flux::Vector{Float64}, in_fluxe::Vector{Float64}, wave_filter::Matrix{Float64}, d::disk_parameter_update)
    ...
    Threads.@threads for i=1: size(angle_zones, 2)
    ...
    Threads.@threads for i=1:length(tmp_temperature)
    ...
    Threads.@threads for i = 1: length(tem_ori)
    ...
    lighttravel_time = sqrt.(d.hcor ^2 .+ rmid .^ 2) .- rmid .* cos.(angle) * sin(d.disk_ia) .+ d.hcor * cos(d.disk_ia)
    ...
    star_i = 0
    star_j = star_i + 20 *obs_per_day
    end_index = length(in_flux) - 20 * obs_per_day
...
function reprocessing(td::Any, items::Any, d::disk_parameter_update) # td: thin_disk return parameter
    ...
    return (idx = items[1], ary=[td.in_time[index], sum(band_flux["uw2"]),sum(band_flux["um2"]),sum(band_flux["uw1"]),sum(band_flux["uuu"]),sum(band_flux["ubb"]), sum(band_flux["uvv"])])
end